        ...
        QSplitter(parent), m_type(p_viewType), m_campaign(p_campaign)
{
    ...
    if (m_type != Type_Order && m_type != Type_Order2 && m_type != Type_Delivery && m_type != Type_Delivery2)
    {
    ...
    connect(m_table->horizontalHeader(), SIGNAL(clicked(QModelIndex)),this,SIGNAL(headerClicked(QModelIndex)));
        connect(m_table->horizontalHeader(), SIGNAL(clicked(QModelIndex)),this,SLOT(debugSlot(QModelIndex)));
    ...
    if(m_type == Type_Order)
        m_table->setItemDelegateForColumn(5, new Delegate(1234));
    if(m_type == Type_Product)
        m_table->setItemDelegate(new Delegate(123456));
}
...
{
    qDebug() << "m_table->currentIndex().row() " << m_table->currentIndex().row();
...
void CAMSEGTable::setupBaseTable()
{
    ...
    switch (m_type)
    {
        ...
        case Type_Order:
            ...
            m_widths << settings.value("3", 140).toInt();
            m_widths << settings.value("4", 85).toInt();
            m_widths << settings.value("5", 85).toInt();
            m_widths << settings.value("6", 95).toInt();
            m_widths << settings.value("7", 95).toInt();
            m_widths << settings.value("8", 95).toInt();
...
void CAMSEGTableHeader::mousePressEvent(QMouseEvent* event)
{
    ...
    qDebug() << "void CAMSEGTableHeader::mousePressEvent(QMouseEvent* event)";
...
void CAMSEGTableHeader::updateTable()
{

}

void CAMSEGTable::debugSlot(QModelIndex index)
{
qDebug() << "void CAMSEGTable::debugSlot()";
}
